                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            <div class="categories-title m-0">
                                <h2><span class="text-primary">{{translate('find')}}</span> {{translate('about_what_you_need')}}</h2>
                            </div>
                            <div>
                                ...
                                @foreach($categories as $key => $category)
                                    @if ($key<10)
                                        <div class="text-center __m-5px __cate-item">
                                            <a href="{{route('products',['id'=> $category['id'],'data_from'=>'category','page'=>1])}}" alt="{{$category->name}}">
                                                <div class="__img">
                                                    <img alt="{{ $category->name }}"
                                                         src="{{ getValidImage(path: 'storage/app/public/category/'.$category->icon, type: 'category') }}">
                                                </div>
                                                <p class="text-center fs-13 font-semibold mt-2">{{Str::limit($category->name, 20)}}</p>
                                            </a>
                                        </div>
                                    @endif
                                @endforeach
                                    ...
                                    @if ($key<10)
                                        <div class="text-center m-0 __cate-item w-100">
                                            <a href="{{route('products',['id'=> $category['id'],'data_from'=>'category','page'=>1])}}" alt="{{$category->name}}">
                                                ...
                                                <p class="text-center small mt-2">{{Str::limit($category->name, 20)}}</p>
                                            </a>
                                        </div>
                                    @endif
                                @endforeach
