...
class UserProfileController extends Controller
{
      ...
      public function my_products(Request $request)
    {
        ...
        $products=Product::where('client_id','=',auth('customer')->id()) ->orderBy('id', $order_by)->paginate(5);
        return view(VIEW_FILE_NAMES['account_my_products'], compact('products', 'order_by'));
    }
        ...
        public function getCategories(Request $request)
    {
        ...
        return view(VIEW_FILE_NAMES['account_my_categories_list'], compact('categories'));
    }
    
    public function my_products_create(Request $request){
        ...
        return view(VIEW_FILE_NAMES['account_my_product_create'], compact('languages', 'request', 'languages', 'defaultLanguage', 'brands', 'brandSetting', 'digitalProductSetting'));

        
    }
    public function my_products_store(ProductAddRequest $request, ProductService $service): JsonResponse|RedirectResponse
    {
        ...
        $dataArray = $service->getAddProductData(request: $request, addedBy: 'admin',client_id:auth('customer')->id());
        ...
        return redirect()->route('account-my-products');
    ...
    {
        $product = $this->productRepo->getFirstWhere(params: ['id' => $id, 'user_id' => 1]);
    
        
        if($product && $product->client_id==auth('customer')->id()){
